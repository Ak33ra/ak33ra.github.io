...
- **`projects/`** — portfolio entries: category (`academic` or `side`), role, stack, dates, links, optional write-up. The index groups projects into stacked "Academic projects" and "Side projects" sections, sorted newest-first within each group and rendered via `ProjectCard`. Implemented.
